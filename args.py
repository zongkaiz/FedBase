...
def input_options():
    ...
    parser.add_argument('--round_num', type=int, default=20, help='number of round in comm')
    ...
    parser.add_argument( '--algorithm', help='algorithm;', choices=OPTIMIZERS, type=str, default='fedprox')
    ...
    dataset = GetDataSet(options['dataset_name'][:5]) # 拿到数据集 分配完再导入 mnist_dir_取前五个即mnist
    client_label, result = dirichlet_split_noniid(dataset.trainLabel, options['dirichlet'], options['num_of_clients'])#client_label:列表，里面每个元素是numpy数组（每个元素内容为每个客户端的样本索引）
    ...
    trainer_path = 'src.fed_server.%s' % options['algorithm']
