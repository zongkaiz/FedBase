...
class BaseFederated(object):
    ...
    def aggregate_parameters(self, local_model_paras_set):
        ...
        print("id(averaged_paras)", id(averaged_paras))
        train_data_num = 0
        ...
        with torch.no_grad():
            for X, y in testDataLoader:
                if self.gpu:
                    X, y = X.cuda(), y.cuda()
                    pred = self.model(X)
                    loss = criterion(pred, y)#criterion=torch.nn.functional.cross_entropy
                    _, predicted = torch.max(pred, 1)

                    correct = predicted.eq(y).sum()#对比模型预测的类别 predicted 与实际标签 y，生成一个布尔张量，其中每个元素表示预测是否正确。sum() 计算出预测正确的样本数量 correct。
    ...
    def getDelay(self, select_clients):
        ...
        # maxD2 = 0
        for client in select_clients :
            if client.getSumDelay() > maxD:
                maxD = client.getSumDelay()
        self.cost.delay_Sum += maxD # 修改成累计时间
