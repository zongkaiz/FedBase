...
class Client():
    ...
    def getLocalEngery(self):
        localEngery = (10 ** -26) * (self.attr_dict['cpu_frequency'] * 1000000000) ** 2 * 10000 * len(self.local_dataset) * self.options['local_epoch']
        return localEngery
    ...
    def getUploadDelay(self):
        R_K = self.attr_dict['B'] * 1000000 * np.log2(1 + self.attr_dict['transmit_power'] * self.attr_dict['g_N0'] / self.attr_dict['B']) # 1M bit / s / self.B
