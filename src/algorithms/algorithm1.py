...
class Algorithm():
    ...
    def getProba(self):
        ...
        sum1 = 0.05 * sum(C_SCORE_N) + 0.05 * sum(B_SCORE_N) + 0.9 * sum(D_SCORE_N)

        proba = [(0.05 * C_SCORE_N[i] + 0.05 * B_SCORE_N[i] + 0.9 * D_SCORE_N[i]) / sum1  for i in range(len(C_SCORE_N))]
        # print(sum(proba))
